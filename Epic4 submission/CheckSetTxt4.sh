...
#===================================================================================
#
# FILE: CheckSetTxt4.sh
#
# USAGE (Cygwin): bash .\CheckSetTxt4.sh FilePrefix
#
# DESCRIPTION: Put this script in the same directory as InCollege.cob and the Makefile,
#   then specify FilePrefix in the commandline arguments.  Set the FOLDER variables
#   appropriately beforehand.  Script will run all input.txt
#   with filenames prefixed by FilePrefix, skipping cleaning storage files if filename
#   has "-{cont}" postfix.  Will check if terminal output matches output.txt, if output.txt
#   matches already existing output.txt, and will move output.txt to CHECK_FOLDER if it
#   doesn't exist there.  If output.txt doesn't match existing output.txt, the diff
...
SOURCE_FOLDER="$WORK_DIR/Epic4 submission folders/Epic4-Storyx-Test-Input"
CHECK_FOLDER="$WORK_DIR/Epic4 submission folders/Epic4-Storyx-Test-Output"
DIFF_FOLDER="$WORK_DIR/TempDiff"

# Filename prefix passed in as commandline argument
FILENAME="$1"
...
for file in "$SOURCE_FOLDER"/$1*.txt; do
    [ -e "$file" ] || exit 1  # Skip if no .txt files
    ...
    if diff -q --strip-trailing-cr "$WORK_DIR/InCollege-Output.txt" "$CHECK_FOLDER/${BASENAME}-Output.txt"; then
        echo "${BASENAME} output file matches."
    else
        #echo "${BASENAME} does not match!!!"
        if [[ -e "$CHECK_FOLDER/${BASENAME}-Output.txt" ]]; then
            diff -q --side-by-side --suppress-common-lines --strip-trailing-cr $WORK_DIR/InCollege-Output.txt "$CHECK_FOLDER/${BASENAME}-Output.txt" > "$DIFF_FOLDER/${BASENAME}-Diff.txt"
        else
            # Copy and rename InCollege-Output.txt to check folder with same basename
            cp "$WORK_DIR/InCollege-Output.txt" "$CHECK_FOLDER/${BASENAME}-Output.txt"
            echo "Creating $BASENAME-Output.txt"
        fi
    fi
done
